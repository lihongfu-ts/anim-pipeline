...
<p align="center">
  <img src="docs/demo2_portrait.png" width="200" alt="立绘">
  &nbsp;
  <img src="docs/demo2_walk.gif" width="110" alt="走路循环">
  &nbsp;
  <img src="docs/demo2_walk.png" width="520" alt="走路图集">
  <br>
  <sub>↑ 2026-09-09 网页端真出的：一句话「国风水墨画风格的年轻刀客」→ 立绘 ¥0.05 → 走路 6 帧循环 ¥0.40，斜俯视视角。左：立绘；中：循环预览；右：接进游戏的图集（6 列 × 192×256）</sub>
</p>
...
> 以 Windows 为主；macOS / Linux 看下面「装」里的手动步骤。
...
**③ 启动**：双击 `启动.bat`，浏览器自动打开 <http://127.0.0.1:8765>。关掉那个黑窗口就是停服务。

**④ 首次配置**（页面会拦着你先填齐，之后不用再碰）

| 要配的 | 干什么 | 去哪拿 | 价格 |
|---|---|---|---|
| **万相**（必需） | 出视频 | <https://platform.qianwenai.com/try-ai?scene=video> → 「万相 3.0 - 视频生成」→ 右上角「获取 API Key」（`sk-ws-` 开头） | 480P/2s ¥0.40，5s ¥1.05 |
| **大模型**（必需） | 写提示词、看联络表挑帧和质检 | <https://platform.deepseek.com>，模型填 `deepseek-v4-pro`（要能看图；v4-flash 不行） | 每次几分钱 |
| **中转站**（出立绘要；已有立绘可不配） | 出立绘、图生图改版 | **关键：必须支持 `gpt-image-2.5`**。推荐 <https://jizhiapi.site/register?aff=9GKRGQJNL4R4>（邀请码 `9GKRGQJNL4R4`）：baseUrl `https://jizhiapi.site/v1`，model `gpt-image-2.5`，价格填 0.05 | ¥0.05/张 |

每家填完点「检查」，绿了再往下。key 写在本机 `~/.gamegen/creds.json`，不进仓库、不上传。

**⑤ 出第一个角色**

1. 选「视角」：**横版（侧视）** 或 **斜俯视地图 + 侧视角色**（传奇类 H5 / 放置刷怪那种）。两种都只有一个朝向，游戏里左右靠镜像
2. 一句话描述角色（例：`一个刀客，国风水墨画风格`）→「出立绘」→ 大模型先把它写成具体的外观描述给你看，能改、能让它重写 →「确认出图」（¥0.05）
3. 不满意 →「再出一张」；想微调 →「改一版…」写改什么（同样先看整理后的指令再出）。满意 →「满意，选动作」
4. 勾动作：**待机 / 移动 / 奔跑 / 受击 / 死亡 / 攻击**（攻击 ⚙ 里可选 1~3 段连招）→「写提示词 → 出动作」
5. 审阅卡：每个动作一段运动描述（大模型按这张立绘写的），看一眼、想改就改 →「确认出片」（每个动作 ¥0.40~1.05）
6. 每个动作等 1~2 分钟。结果卡片：GIF、指标、👁 视觉模型的结论（判废会说原因，并停掉后面的动作不再花钱）
7. 帧数不合适 →「重切」（用已出的视频重挑帧，不花钱）；「预览 / 调顿帧」看循环、调每帧时长；「导出 ZIP」拿图集 + 逐帧 PNG + `manifest.json`（格子、帧数、footTrim、命中帧、取消窗口、连招顺序）接引擎

**已经有立绘**：「不出图，用已有立绘」→ 上传（灰底、全身、朝左）或从作品里选，跳过 ¥0.05。

**一个角色大概多少钱**：立绘 ¥0.05 + 待机 / 移动 / 奔跑 / 受击各 ¥0.40 + 攻击 / 死亡各 ¥1.05 ≈ **¥3.75**。废片重出按 ×1.5 做预算。

**更新代码**：`git pull`（或重新下 ZIP 覆盖）后，网页右上角会自己亮出「↻ 重启服务」，点一下就行；有任务在跑时会拒绝。
...
### 实测 ②：刀客 · 斜俯视视角 · 提示词先审后出（2026-09-09 晚）

<p align="center">
  <img src="docs/demo2_walk.gif" width="110" alt="走路"> &nbsp; <img src="docs/demo2_hurt.gif" width="120" alt="受击">
  &nbsp;&nbsp; <img src="docs/demo2_hurt.png" width="420" alt="受击图集">
</p>

| 动作 | 帧 | 动量 | 形变 | 循环缝 | 亮度漂 | 位移 | 视觉模型 |
|---|---|---|---|---|---|---|---|
| 走路（loop，重切成 6 帧） | 6 | 30.6 | 20.9% | 1.32 | 4.0 | −0.9% / 抬升 3.7% | 动作对、没崩 |
| 受击（DeepSeek 挑 1/20/30/60，重切成 6 帧） | 6 | 21.3 | 12.6% | — | 2.6 | — | 没崩，但备注写的是「闪避缩身」 |

走路可用：斗笠、飘带、刀全程稳，俯视镜头没被拉平。**受击不算合格**——模型把「头肩后缩、重心后坐」演成了**下沉举刀的防御架势**，
看不出被打中；视觉模型注意到了（备注「闪避」）却没判它不对，因为当时一次性动作的规则只管挑帧和崩坏。
这一发之后改了两处：一次性动作也让视觉模型判「演的是不是这个动作」；受击模板改成「被击退」的受力写法（身体向背后弹开半步、头一甩、飘带向面朝方向甩出、双臂被震开）并明确禁止摆架势。
给视觉模型看的联络表同时从 3840×2560 PNG（5.5 MB、DeepSeek 要 92 秒）缩成 2560 宽 JPEG（0.3 MB）。
...
**Windows 一键**：双击 `安装.bat`（装 Python 依赖 + ffmpeg，没 Python 会先用 winget 装），装完双击 `启动.bat` 打开网页。

手动：

```bash
pip install -r requirements.txt      # numpy + Pillow，就这两个（网页端再加 web/requirements.txt）
python tools/install_ffmpeg.py       # ffmpeg / ffprobe：有 winget 用 winget，否则下 gyan.dev 的包解压到 tools/bin
```

ffmpeg 也能在网页「设置 → 环境」里点「一键安装」，装完不用重启。

macOS / Linux：`pip install -r requirements.txt -r web/requirements.txt`，`brew install ffmpeg` / `sudo apt install ffmpeg`，
然后 `python web/server.py`。

三家视频/图像 API 的调用是标准库 urllib 手写的，没有任何厂商 SDK。
...
跑完去看 `out/anim/demo_walk.png`（图集）和 `work/anim/demo_walk_看.gif`（预览）——长这样：

<p align="center">
  <img src="docs/demo_walk.gif" width="100" alt="demo 循环预览">
  &nbsp;&nbsp;
  <img src="docs/demo_walk.png" width="400" alt="demo 序列帧图集">
</p>
...
```bash
pip install -r web/requirements.txt      # fastapi / uvicorn / python-multipart（或直接双击 安装.bat）
python web/server.py                     # → http://127.0.0.1:8765（或双击 启动.bat）
```

**中转站的关键要求：必须支持 `gpt-image-2.5`** —— 出立绘和图生图改版都靠它，别的图像模型没实测过；
保存凭据后点「检查」，模型列表里没有它就换一家。

五个页签：**制作 · 作品 · 预览 · 设置 · Agent**（操作步骤见上面「快速开始 ⑤」）

- **制作**：视角 → 一句话 → 出立绘 → 选动作 → 出片 → 图集 → 验收。
  六个常用动作：待机 / 移动 / 奔跑 / 受击 / 死亡 / 攻击（1~3 段连招：横砍 → 竖劈 → 直刺，三种剪影分得开，是文档 §7 那套教训的直接应用）。
  **提示词两道审**：立绘的外观描述、每个动作的运动段，都由大模型按这张立绘写好、给你看、能改能重写，确认了才花钱。
  一句话留空时会先让视觉模型看立绘写角色描述（否则大模型不知道手里有剑，会把武器写丢）。
  **出片后视觉模型质检**：一次性动作挑帧 + 判崩坏；循环动作判「演的是不是这个动作」+ 判崩坏（腿变棍、剑消失、踢腿当奔跑都抓得到）。
  判废先把图集和 GIF 出完给你看，再停后面的动作。
  **每个动画独立记账**：预估 / 实花分开，视频按「任务 id 落盘时刻」记——片取不回钱照扣。
  **重切**：用已出的视频换帧数重新挑帧，不花钱。
  **试运行**开关：合成素材走后半段，不调任何 API，用来测链路。
- **作品**：所有任务的产物、花费、日志；「用这张立绘出动作」复用立绘（视角跟着立绘走）。
- **预览**：Canvas 逐帧播图集，fps、**逐帧 holds（顿帧）**、洋葱皮看循环缝、像素放大；旁边是五项验收指标、位移、图集自检、
  视觉模型结论、运动段来源、这个动画花了多少钱；下载图集 / GIF / ZIP；也能上传任意图集预览和切帧。
- **设置**：环境（ffmpeg 一键安装）+ 各家 key（写 `~/.gamegen/creds.json`，不回显完整 key），每家一个「检查」。
- **Agent**：让大模型自己跑完整条链（写运动段、选 provider、读验收数字、决定重出还是换路线），收费步骤停下来问你。

服务只监听 127.0.0.1。管线代码一行不动——每一步都是 subprocess 调 `tools/` 里那份脚本，每个任务一个独立目录。
代码更新后网页右上角亮「↻ 重启服务」，点一下即可。支持哪些 2D 游戏类型、以后怎么扩到多朝向，见
[docs/2D游戏类型与视角参考.md](docs/2D游戏类型与视角参考.md)。
